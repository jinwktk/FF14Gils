...
- 表示は一覧とグラフを切り替えできます。グラフ画面は同じ絞り込み結果から、売上額トップ、状態別売上、価格変動を表示します。
...
グラフ画面は追加APIを呼ばず、読み込み済みスナップショットの `items` を `src/marketshare.js` で集計して描画します。
...
定期実行は毎時17分の schedule です。GitHub Actions の schedule は遅延または間引きされる場合があるため、厳密な1時間更新を保証するものではありません。
