...
describe('translate', () => {
  it('日本語と英語のUI文言を返す', () => {
    ...
    assert.equal(translate('ja', 'ranking.title'), 'ワールド売上ランキング');
    assert.equal(translate('en', 'ranking.title'), 'World sales ranking');
    assert.equal(translate('ja', 'ranking.region'), 'リージョン');
    assert.equal(translate('en', 'ranking.region'), 'Region');
    assert.equal(translate('ja', 'ranking.topItem'), '売上上位');
    assert.equal(translate('en', 'ranking.topItem'), 'Top item');
    assert.equal(translate('ja', 'nav.market'), '金策候補');
    assert.equal(translate('en', 'nav.ranking'), 'World ranking');
    assert.equal(translate('ja', 'nav.legal'), '権利表記とデータ');
    assert.equal(translate('en', 'nav.legal'), 'Rights and data');
    assert.equal(translate('ja', 'legal.title'), '権利表記とデータについて');
    assert.equal(translate('en', 'legal.title'), 'Rights and data');
    assert.equal(translate('ja', 'legal.sourcesTitle'), '利用しているデータ元');
    assert.equal(translate('en', 'legal.sourcesTitle'), 'Data sources');
    assert.match(translate('en', 'legal.unofficialText'), /not an official service/);
    assert.match(translate('en', 'legal.dataHandlingBrowser'), /does not POST directly/);
